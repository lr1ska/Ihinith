...
load_dotenv()
GOOGLE_AI_KEY = "AIzaSyCqzdw49RQniEkixLGZX3tIwX7oDaQVUW0"
DISCORD_BOT_TOKEN = "MTMwODAyNjAxNTAyMDIyNDU2NA.GwFpB9.ts0-wyRHwGPkcj1aLI46zrAJ1Yqdsrz7bRM1TI"
MAX_HISTORY = 300
ALLOWED_CHANNEL_ID = 1215861775052963980
TARGET_CHANNEL_ID = 1216957364788400269
...
@bot.event
async def on_ready():
    game = discord.Game(name="Vinland", url="https://discord.gg/upaRfqkFzQ")
    await bot.change_presence(status=discord.Status.idle, activity=game)
    #await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="VinLand"))
    print("----------------------------------------")
...
async def on_message(message):
    ...
    if message.channel.id == TARGET_CHANNEL_ID:
        #user_mentioned = mention.group(0)
        try:
            response = f"{message.author.mention} Радий що ти до нас завітав!\nПреглянь канали <#1216957228196691968> і <#1215860664791470161>,\nтам ти знайдеш усю інформацію про сервер і як почати грати на проєкті."
            await message.channel.send(response)
        except Exception as e:
            print(f"Помилка: {e}")
        
        #await message.channel.send("Задане мною повідомлення")
        return  # Игнорируем сообщения, если не в нужном канале    

    # Обработка запроса "как зайти на сервер"
    trigger_phrase = "як зайти на сервер"
    trigger_phrase_regex = re.compile(r"\b" + trigger_phrase + r"\b", re.IGNORECASE)

    # Более толерантный поиск
    variations = ["як попасти на сервер", "як ввійти на сервер", "як приєднатися до серверу", "інструкція по входу на сервер", "як приєднатися на сервер"]
    variations_regex = [re.compile(r"\b" + variation + r"\b", re.IGNORECASE) for variation in variations]

    if any(regex.search(message.content) for regex in variations_regex):
      response = "Вот инструкция по входу на сервер:\n\n[Вставьте здесь инструкцию]"
      await message.channel.send(response)
      return

if trigger_phrase_regex.search(message.content):
        response = "Вот инструкция по входу на сервер:\n\n[Вставьте здесь инструкцию]"
        await message.channel.send(response)
        return
#----------
